...
#endif
'''
extra = anchor + '''#ifdef REMOTE_GPIO_OUT4
...
commands = r'''#ifdef REMOTE_GPIO_OUT4
...
extra = r'''#ifdef REMOTE_GPIO_OUT4
...
# Enable only one additional test output: GPIO38.
p = Path("variants/heltec_v3/platformio.ini")
s = p.read_text()
needle = '  -D REMOTE_GPIO_IN2=7\n'
replacement = needle + '  -D REMOTE_GPIO_OUT4=38\n'
count = s.count(needle)
if count < 2: raise SystemExit(f"expected V3 flags twice, found {count}")
s = s.replace(needle, replacement)
p.write_text(s)

print("Heltec V3 isolated GPIO38 test: OUT4=GPIO38; V3 OUT1/OUT2 + IN1/IN2 retained")
