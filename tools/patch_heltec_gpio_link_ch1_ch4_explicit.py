...
if anchor not in s: raise SystemExit("LINK CH1-CH4 globals anchor not found")
...
block = r'''  bool gpio_command_recognized = false;
  ...
  if (gpio_link_sender_ok && strcmp(text, "LINK OUT4 ON") == 0) {
    gpio_out4_timer_expiry = 0;
    gpio_out4_timer_contact_valid = false;
    digitalWrite(REMOTE_GPIO_OUT4, HIGH);
    gpio_reply = "LINK ACK OUT4 ON";
    gpio_command_recognized = true;
  } else if (gpio_link_sender_ok && strcmp(text, "LINK OUT4 OFF") == 0) {
    gpio_out4_timer_expiry = 0;
    gpio_out4_timer_contact_valid = false;
    digitalWrite(REMOTE_GPIO_OUT4, LOW);
    gpio_reply = "LINK ACK OUT4 OFF";
    gpio_command_recognized = true;
  }
#endif

'''
if anchor not in s: raise SystemExit("LINK CH1-CH4 command anchor not found")
...
loop = r'''#ifdef REMOTE_GPIO_IN1
  ...
  if (_cli_rescue) {
'''
if anchor not in s: raise SystemExit("LINK CH1-CH4 loop anchor not found")
s = s.replace(anchor, loop, 1)
p.write_text(s)
print("Applied explicit LINK CH1+CH2+CH3+CH4 diagnostic patch")
