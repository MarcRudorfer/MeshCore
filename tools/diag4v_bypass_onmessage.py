...
# Find the opening brace and matching closing brace robustly, regardless of prior DIAG patches.
brace = s.find('{', start)
if brace < 0:
...
s = s[:start] + new + s[end:]
p.write_text(s)
print('DIAG4V: onMessageRecv fully bypassed (robust function replacement)')
