...
# Add OUT3/OUT4 command handling before STATUS.
...
commands = r'''
  ...
  if (strcmp(text, "OUT4 ON") == 0) {
    gpio_out4_timer_expiry = 0;
    gpio_out4_timer_contact_valid = false;
    ...
    if (valid) {
      digitalWrite(REMOTE_GPIO_OUT4, HIGH);
      gpio_out4_timer_expiry = futureMillis(seconds * 1000UL);
      gpio_out4_timer_contact = from;
      gpio_out4_timer_contact_valid = true;
      snprintf(gpio_reply_buf, sizeof(gpio_reply_buf), "OUT4 = ON (TIMER %luS)", seconds);
      gpio_reply = gpio_reply_buf;
    } else {
      gpio_reply = "OUT4 TIMER: USE 1-3600S";
    }
  }
#endif
...
# Expand STATUS to all 8 I/Os and increase reply buffer.
...
#ifdef REMOTE_GPIO_OUT4
  digitalWrite(REMOTE_GPIO_OUT4, LOW);
  pinMode(REMOTE_GPIO_OUT4, OUTPUT);
  gpio_out4_timer_expiry = 0;
  gpio_out4_timer_contact_valid = false;
#endif
''' + anchor
...
# Initialize IN3/IN4 before notify reset.
anchor = '  gpio_notify_contact_valid = false;\n  BaseChatMesh::begin();'
extra = '''#ifdef REMOTE_GPIO_IN3
  pinMode(REMOTE_GPIO_IN3, INPUT_PULLUP);
  gpio_in3_stable = digitalRead(REMOTE_GPIO_IN3);
  gpio_in3_raw = gpio_in3_stable;
  gpio_in3_debounce_expiry = 0;
#endif
#ifdef REMOTE_GPIO_IN4
  pinMode(REMOTE_GPIO_IN4, INPUT_PULLUP);
  gpio_in4_stable = digitalRead(REMOTE_GPIO_IN4);
  gpio_in4_raw = gpio_in4_stable;
  gpio_in4_debounce_expiry = 0;
#endif
''' + anchor
if anchor not in s: raise SystemExit("begin IN extension anchor not found")
s = s.replace(anchor, extra, 1)

# Add OUT3/OUT4 timer completion before input processing.
...
#endif
#ifdef REMOTE_GPIO_OUT4
  if (gpio_out4_timer_expiry && millisHasNowPassed(gpio_out4_timer_expiry)) {
    digitalWrite(REMOTE_GPIO_OUT4, LOW);
    gpio_out4_timer_expiry = 0;
    if (gpio_out4_timer_contact_valid) {
      uint32_t expected_ack = 0;
      uint32_t est_timeout = 0;
      sendMessage(gpio_out4_timer_contact, getRTCClock()->getCurrentTimeUnique(), 0,
                  "OUT4 = OFF (TIMER DONE)", expected_ack, est_timeout);
      gpio_out4_timer_contact_valid = false;
    }
  }
#endif
...
# Add IN3/IN4 change notification before CLI handling.
anchor = '  if (_cli_rescue) {\n'
inputs = r'''#ifdef REMOTE_GPIO_IN3
  {
    int raw = digitalRead(REMOTE_GPIO_IN3);
    if (raw != gpio_in3_raw) {
      gpio_in3_raw = raw;
      gpio_in3_debounce_expiry = futureMillis(50);
    }
    if (gpio_in3_debounce_expiry && millisHasNowPassed(gpio_in3_debounce_expiry)) {
      gpio_in3_debounce_expiry = 0;
      if (gpio_in3_stable != gpio_in3_raw) {
        gpio_in3_stable = gpio_in3_raw;
        if (gpio_notify_contact_valid) {
          uint32_t expected_ack = 0;
          uint32_t est_timeout = 0;
          sendMessage(gpio_notify_contact, getRTCClock()->getCurrentTimeUnique(), 0,
                      gpio_in3_stable == LOW ? "IN3 = ON" : "IN3 = OFF",
...
#ifdef REMOTE_GPIO_IN4
  {
    int raw = digitalRead(REMOTE_GPIO_IN4);
    if (raw != gpio_in4_raw) {
      gpio_in4_raw = raw;
      gpio_in4_debounce_expiry = futureMillis(50);
    }
    if (gpio_in4_debounce_expiry && millisHasNowPassed(gpio_in4_debounce_expiry)) {
      gpio_in4_debounce_expiry = 0;
      if (gpio_in4_stable != gpio_in4_raw) {
        gpio_in4_stable = gpio_in4_raw;
        if (gpio_notify_contact_valid) {
          uint32_t expected_ack = 0;
          uint32_t est_timeout = 0;
          sendMessage(gpio_notify_contact, getRTCClock()->getCurrentTimeUnique(), 0,
                      gpio_in4_stable == LOW ? "IN4 = ON" : "IN4 = OFF",
                      expected_ack, est_timeout);
        }
      }
    }
