...
static bool adc1_alarm_contact_valid = false;
static ContactInfo adc1_alarm_contact;
...
static uint32_t readGpioAdcMilliVolts(uint8_t pin) {'''
if anchor not in s:
    raise SystemExit("ADC helper anchor not found")
s = s.replace(anchor, insert, 1)

# Add commands before STATUS. The sender of any ADC alarm command becomes the alarm contact.
...
cmds = '''#ifdef REMOTE_ADC_IN1
  if (strcmp(text, "ADC1 ALARM ON") == 0) {
    adc1_alarm_contact = from; adc1_alarm_contact_valid = true;
    ...
    snprintf(gpio_reply_buf, sizeof(gpio_reply_buf), "ADC1 %lumV | ALARM=%s %s | LOW=%umV HIGH=%umV HYST=%umV REPEAT=%lus", (unsigned long)mv, adc1_alarm_enabled?"ON":"OFF", adc1_alarm_active?"ACTIVE":"OK", adc1_low_mv, adc1_high_mv, adc1_hyst_mv, (unsigned long)adc1_repeat_sec);
...
# Insert periodic alarm handling before existing OUT1 timer processing.
...
loop_code = '''#ifdef REMOTE_ADC_IN1
  if (adc1_alarm_enabled && adc1_alarm_contact_valid && millis() - adc1_last_check_ms >= 1000UL) {
    adc1_last_check_ms = millis();
    const uint32_t mv = readGpioAdcMilliVolts(REMOTE_ADC_IN1);
    ...
    if (!adc1_alarm_active && trip) {
      ...
      char msg[96];
      if (lowTrip) snprintf(msg, sizeof(msg), "ADC1 ALARM LOW: %lumV < %umV", (unsigned long)mv, adc1_low_mv);
      else snprintf(msg, sizeof(msg), "ADC1 ALARM HIGH: %lumV > %umV", (unsigned long)mv, adc1_high_mv);
      uint32_t expected_ack=0, est_timeout=0;
      sendMessage(adc1_alarm_contact, getRTCClock()->getCurrentTimeUnique(), 0, msg, expected_ack, est_timeout);
    } else if (adc1_alarm_active) {
      const uint32_t lowClear = (uint32_t)adc1_low_mv + adc1_hyst_mv;
      const uint32_t highClear = adc1_high_mv > adc1_hyst_mv ? (uint32_t)adc1_high_mv - adc1_hyst_mv : 0;
      const bool lowRecovered = (adc1_low_mv == 0 || mv >= lowClear);
      const bool highRecovered = (adc1_high_mv == 0 || mv <= highClear);
      if (lowRecovered && highRecovered) {
        adc1_alarm_active = false;
        char msg[64]; snprintf(msg, sizeof(msg), "ADC1 OK: %lumV", (unsigned long)mv);
        uint32_t expected_ack=0, est_timeout=0;
        sendMessage(adc1_alarm_contact, getRTCClock()->getCurrentTimeUnique(), 0, msg, expected_ack, est_timeout);
      } else if (adc1_repeat_sec > 0 && millis() - adc1_last_alarm_ms >= adc1_repeat_sec * 1000UL) {
        adc1_last_alarm_ms = millis();
        char msg[96]; snprintf(msg, sizeof(msg), "ADC1 ALARM STILL ACTIVE: %lumV", (unsigned long)mv);
        uint32_t expected_ack=0, est_timeout=0;
        sendMessage(adc1_alarm_contact, getRTCClock()->getCurrentTimeUnique(), 0, msg, expected_ack, est_timeout);
      }
    }
