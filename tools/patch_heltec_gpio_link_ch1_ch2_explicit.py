...
if anchor not in s: raise SystemExit("LINK CH1-CH2 globals anchor not found")
...
'''
if anchor not in s: raise SystemExit("LINK CH1-CH2 command anchor not found")
...
loop = r'''#ifdef REMOTE_GPIO_IN1
  ...
  if (_cli_rescue) {
'''
if anchor not in s: raise SystemExit("LINK CH1-CH2 loop anchor not found")
s = s.replace(anchor, loop, 1)
p.write_text(s)
print("Applied explicit LINK CH1+CH2 diagnostic patch")
